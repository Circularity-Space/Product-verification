...
let userProfile = ( function() {
		...
		let _user_info = (storeHub.getItem("userProfile")) ? JSON.parse(storeHub.getItem("userProfile")) : { "pref":[]}
		storeHub.setItem("userProfile",JSON.stringify({"pref":[]}))

		let saveData = ()=>{
			storeHub.setItem(
						"userProfile",JSON.stringify(_user_info)
			 )
		}
		...
		obj.get_userinfo = ()=>{return _user_info;}
		obj.get_field = (field) =>{return _user_info[field]}
		obj.clearData = ()=>{_user_info={"pref":[]};saveData();}
		return obj
})()
...
let checkDataValid = (info)=>{return (info !== "")? true:false}
...
_boolean[0].addEventListener("click",(e)=>{userProfile.update_userinfo({"gender":"m"})})
_boolean[1].addEventListener("click",(e)=>{userProfile.update_userinfo({"gender":"f"})})
let checkIfFilled = (info)=>{
	if (userProfile.get_field(info) !== "" && userProfile.get_field(info) != undefined) {
			return true
	}
	return false;
}
...
_all_next[2].addEventListener("click",(e)=>{
	...
	if (age !== undefined && age !=="" && checkIfFilled("age") !=="" && checkIfFilled("age") !==undefined) {
		if (_curren_view==0){
			_all_children[_curren_view].setAttribute("style","display:none")
			_all_children[_curren_view].setAttribute("style","display:block")
			_curren_view+=1
...
fetch_btn.addEventListener("click",(e)=>{console.log("Hello floaters");})
let fetch_more = ()=>{}
